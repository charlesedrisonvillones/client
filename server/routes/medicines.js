...
router.post("/editMedicines", auth, async (req, res) => {
  try {
    ...
    // console.log(stocks)
    const editMedicines = await pool.query(
      "UPDATE medicines SET stocks = $1 WHERE name_id=$2",
      [stocks, name_id]
    );
...
router.post("/medicines", auth, async (req, res) => {
  try {
    ...
    const newMedicine = await pool.query(
      "INSERT INTO medicines (name, stocks, expiration_date) VALUES($1, $2, $3)",
      [name, stocks, expiration_date]
    );
...
router.delete("/medicines/:name_id", auth, async (req, res) => {
  try {
    ...
    const deleteFeed = await pool.query(
      "DELETE FROM medicines WHERE name_id=$1",
      [name_id]
    );
...
router.get("/countMedicines", auth, async (req, res) => {
  try {
