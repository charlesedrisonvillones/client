...
router.post("/pigs", auth, async (req, res) => {
  try {
    ...
    const newPig = await pool.query(
      "INSERT INTO pigs (name, sow, weight, type) VALUES($1, $2, $3, $4)",
      [name, sow, weight, type]
    );
...
router.post("/editPigs", auth, async (req, res) => {
  try {
    ...
    const editPigs = await pool.query(
      "UPDATE pigs SET weight = $1,type = $3 WHERE id=$2",
      [weight, id, type]
    );
...
router.get("/countPigs", auth, async (req, res) => {
  try {
    const count = await pool.query("SELECT count(*) from pigs");
    // console.log(count);
